...
- **Supabase:** project `bbc-platform` live at `https://irtlnvkrmbzkervogczp.supabase.co` (US East). `profiles` table with RLS policies applied. RLS is defense-in-depth (Phase 1); all client access routes through BFF with `service_role` key. JWT verification (Keycloak RS256 → Supabase) deferred to Phase 2.
